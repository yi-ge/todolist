...
class Board
{
    ...
    public function newBoard()
    {
        ...
        $manageUsername = $this->f3->get('PARAMS.username');

        if (!empty($manageUsername)) {
            do {
            ...
            if ($this->db->exec("INSERT INTO boards (name, manageUsername) VALUES (?, ?)", $new_name, $manageUsername)) {
                $result = array(
                    "status" => 1,
                    "result" => array(
                        "name" => $new_name
